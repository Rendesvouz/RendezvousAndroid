...
const LoginScreen = ({ navigation, route }) => {
          ...
          .then((res) => {
            ...
            if (res?.data) {
              ...
              if (res?.data?.role == "User") {
                ...
                RNToast(
                  Toast,
                  "Please use the website to keep enjoying our services"
                );
                // Redirect other roles to the web browser
                Linking.openURL(rendezvousWebsiteURL).catch((err) =>
  ...
  const checkUserProfile = async (access_token, refresh_token) => {
    try {
      ...
      if (profileResponse?.data?.data && profileResponse?.data?.data?.profile) {
        ...
        checkPointFromLogin(navigation, profileResponse?.data?.data, item);
  ...
  const checkUserPreferences = async (access_token, userProfileData) => {
    try {
      if (!access_token) {
        navigation.navigate("LoginScreen");
        return;
      ...
      const userId = userProfileData?.profile?.user_id;
      ...
      if (preferenceResponse?.data) {
        ...
        // dispatch(getUser(userProfileData));
  ...
  return (
    <SafeAreaViewComponent>
      <KeyboardAvoidingComponent>
        ...
        <FormInput
          formInputTitle={"Email or Username"}
          value={email}
